(function () {
  ...
  Array.from(forms).forEach(function (form) {
    // Check if the form is within '.g-recaptcha' and bypass the validation if true.
    if (form.closest(".g-recaptcha") !== null) return;
    ...
    inputs.forEach((input) => {
      input.addEventListener("input", function (event) {
        // Bypass validation for inputs within '.g-recaptcha'.
        if (input.closest(".g-recaptcha") !== null) return;
      ...
      input.addEventListener("blur", function (event) {
        // Bypass blur validation for inputs within '.g-recaptcha'.
        if (input.closest(".g-recaptcha") !== null) return;
  ...
  switch (input.id) {
    ...
    case "message":
      errorMessage =
        input.value === ""
          ? "Message is required!"
          : "Minimum length of message is 20 characters.";
